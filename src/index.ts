...
function formatBearingTable(items: Bearing[], total: number, page: number): string {
  if (items.length === 0) {
  ...
  const rows = items.map(
    (b) =>
      `| ${b.partNumber || "-"} | ${b.brandName || "-"} | ${b.innerDiameter ?? "-"} | ${b.outerDiameter ?? "-"} | ${b.width ?? "-"} | ${b.bearingTypeName || "-"} |`
  );
...
export default definePluginEntry({
  ...
  register(api) {
    api.registerTool({
      name: "bearing_search",
      description:
        "搜索轴承产品。支持按型号关键词、品牌、内径/外径/宽度尺寸范围等条件搜索。用户说「搜轴承」「查轴承」「找轴承」时使用此工具。",
      parameters: SearchParams,
      async execute(_id, params) {
        try {
          const searchParams: Record<string, string> = {};
          for (const [k, v] of Object.entries(params)) {
            if (v !== undefined && v !== "") searchParams[k] = String(v);
          }
          const res = await apiFetch<Bearing[]>("bearings/search", searchParams);
    ...
    api.registerTool({
      ...
      description:
        "按精确型号查询轴承详情。用户给出明确型号（如 60025、6205-2RS）时使用此工具获取完整参数。",
      parameters: PartNumberParams,
      async execute(_id, rawParams) {
        const params = rawParams as Static<typeof PartNumberParams>;
        try {
          const res = await apiFetch<Bearing>(`bearings/by-part/${encodeURIComponent(params.partNumber)}`);;
          if (!res.data) {
          ...
          if (b.brandCountry) lines.push(`品牌产地：${b.brandCountry}`);
          if (b.originCountry) lines.push(`产地：${b.originCountry}`);
          if (b.weight !== undefined && b.weight !== null) lines.push(`重量：${String(b.weight)} kg`);
    ...
    api.registerTool({
      ...
      async execute(_id, params) {
        try {
          ...
          const res = await apiFetch<Bearing[]>("bearings/hot", query);
          const items = res.data || [];
